	...
	public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
		...
		if(event.getName().equals("help")){
			...
			if(event.getMember().getId().equals(auth)){
				serverList.insertOne(new Server(name, bigname, ip).toDocument()); //adds to the "mc" collection which stores server info
			...
			try {
				...
				ProcessBuilder pb = new ProcessBuilder("ssh", "-tt", "-i", "~/.ssh/ssh-key-2022-06-18.key", "opc@" + server.getIp(), "tmux", "send-keys", "'~/" + server.getName() + "/minecraft/run.sh'", "Enter", "C-m").inheritIO();
			...
			try {
				...
				ProcessBuilder pb = new ProcessBuilder("ssh", "-tt", "-i", "~/.ssh/ssh-key-2022-06-18.key", "opc@" + server.getIp(), "tmux", "send-keys", "/stop", "Enter", "C-m").inheritIO();
				Process p = pb.start();
			} catch (IOException e) {
				event.getHook().sendMessage("IOException").queue();
				e.printStackTrace();
